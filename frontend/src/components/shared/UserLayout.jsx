...
const NAV = [
  { to: '/dashboard',         icon: GridIcon,    label: 'Dashboard',    end: true },
  { to: '/dashboard/books',   icon: BookStack,   label: 'Browse Books' },
  { to: '/dashboard/history', icon: HistoryIcon, label: 'My Borrows' },
  { to: '/dashboard/chatbot', icon: PenNib,      label: 'AI Assistant' }, 
    ...
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--c-bg)' }}>
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-30 w-[240px] flex flex-col
        ...
        <div className="flex items-center gap-3 px-6 py-5"
          ...
          <div>
            ...
            <p className="text-[10px] uppercase tracking-widest mt-0.5" style={{ color: 'rgba(255,255,255,.3)' }}>Member Portal</p>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 space-y-0.5 overflow-y-auto">
          {NAV.map(({ to, icon: Icon, label, end }) => (
            <NavLink key={to} to={to} end={end} onClick={() => setSideOpen(false)}
              ...
              <Icon size={17} color="currentColor"/> {label}
            </NavLink>
          ))}
        </nav>
        ...
        <div className="px-3 pb-2" style={{ borderTop: '1px solid rgba(255,255,255,.06)' }}>
          <button onClick={() => { setShowNotif(true); setSideOpen(false) }}
            className="nav-item w-full" style={{ marginTop: 8 }}>
            <BellIcon size={17} color="currentColor"/> Notifications
            {unread > 0 && (
              <span className="ml-auto text-[10px] font-bold px-2 py-0.5 rounded-full"
                    style={{ background: 'var(--c-mint)', color: '#fff' }}>{unread}</span>
            )}
          </button>
        </div>
        ...
        <div className="px-3 pb-4">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl"
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold truncate" style={{ color: 'rgba(255,255,255,.9)' }}>{user?.name}</p>
              <p className="text-[10px]" style={{ color: 'rgba(255,255,255,.35)' }}>Member</p>
            </div>
            <button onClick={logout} className="p-1 rounded-lg hover:bg-white/10"
                    style={{ color: 'rgba(255,255,255,.4)' }}>
              <LogOutIcon size={14} color="currentColor"/>
            </button>
          </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="flex items-center gap-4 px-5 lg:px-7 h-[60px] flex-shrink-0"
          ...
          <button className="lg:hidden p-2 rounded-xl" onClick={() => setSideOpen(p => !p)}>
            {sideOpen ? <CloseIcon size={20}/> : <MenuIcon size={20}/>}
          </button>
          <p className="hidden lg:block text-sm" style={{ color: 'var(--c-ink-soft)' }}>
            Welcome back, <span className="font-semibold" style={{ color: 'var(--c-ink)' }}>{user?.name}</span>
          </p>
              ...
              {notifs.length === 0
                ? (
                  <div className="flex flex-col items-center justify-center py-20 gap-3">
                    ...
                    <p className="text-sm" style={{ color: 'var(--c-ink-muted)' }}>All caught up!</p>
                  </div>
                )
                : notifs.map(n => (
                  <div key={n.id}
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs mt-0.5" style={{ color: 'var(--c-ink-soft)' }}>{n.message}</p>
